"""Phase T2.2 Phase E — 3BHK Pune apartment-tower assembler.

Layer-3 building assembler that composes:

  * 1 stilt parking floor (optional, default True) at elevation 0
  * N habitable floors (default 5), each one self-contained 3BHK FLAT
    FloorUnit (Phase B) glued onto the side-core's east edge.
  * 1 flat roof slab on top
  * Continuous RCC columns (300×300mm) spanning -0.5m → top of roof
  * Per-ceiling-level RCC beam grids
  * Isolated pad foundations (one per column, 1.5 × 1.5 × 0.6m)

Reuses `make_2bhk_pune_tower_core()` — the side-core (lobby + stair +
lift) is BHK-agnostic and works for 3BHK without modification. The
flat's east-wall id contract (`-E-s` for habitable storeys with
balcony, `-E` for stilt) is identical to 2BHK FLAT.

Default plot 11.0 × 20.0m yields buildable depth 12.5m — well above
the side-core's `_MIN_CORE_DEPTH_M=8.0` threshold (much more margin
than 1BHK/2BHK towers).

Element scaling (default plot, has_stilt_parking=True):
   habitable_floor_count │ storeys │ walls │ rooms
                       5 │      6  │  113  │   55
                      11 │     12  │  239  │  115
"""
...
# Core column labels at the core's outer corners.
...
def build_3bhk_pune_tower(
    *,
    ...
    wind_zone: int = 2,
    build_id: str = "tier2-3bhk-pune-tower-v1",
    generated_at: str = "2026-05-10T00:00:00Z",
) -> BuildingModel:
    """Build a G+N 3BHK apartment tower (Phase E)."""
    if flats_per_floor != 1:
        raise NotImplementedError(
            f"build_3bhk_pune_tower v1 supports only flats_per_floor=1; "
    ...
    for s_idx, s_id, s_elev, s_h, is_hab in storey_meta:
        ...
        # Add core walls + rooms + 2 core doors per storey.
    ...
    if flat_column_grid_x is None or flat_column_grid_y is None:
        raise RuntimeError(
            "3BHK tower has no habitable storeys — column grid uninitialised"
        )
    ...
    building = Building(
        id="building-1",
        name=f"3BHK Pune Tower G+{habitable_floor_count}",
    ...
    provenance = Provenance(
        ...
        fixture_match=f"tier2-3bhk-pune-tower-g{habitable_floor_count}",
    ...
    project = Project(
        id="project-1",
        name=f"3BHK Pune Tower Project (G+{habitable_floor_count})",
        site=site,
        metadata=metadata,
    )
