"""Phase T2.2 Phase F — Export the 3BHK Pune Tier-2 templates to IFC.

Drives the same `build_ifc_from_building_model` pipeline used by
`export_2bhk_pune_to_ifc.py`, but for all four 3BHK variants in one
run: house, duplex, tower G+5, tower G+11.

Usage:
    python3.11 scripts/export_3bhk_pune_to_ifc.py
"""
...
def main() -> int:
    print("=" * 72)
    print("Phase T2.2 — 3BHK Pune Tier-2 Templates → IFC exports")
    print("=" * 72)
    try:
        _export_one(
            "house",
            build_3bhk_pune_house(),
            _OUT_DIR / "tier2_3bhk_pune_house.ifc",
        )
        _export_one(
            "duplex",
            build_3bhk_pune_duplex(),
            _OUT_DIR / "tier2_3bhk_pune_duplex.ifc",
        )
        _export_one(
            "tower-g5",
            build_3bhk_pune_tower(habitable_floor_count=5),
            _OUT_DIR / "tier2_3bhk_pune_tower_g5.ifc",
        )
        _export_one(
            "tower-g11",
            build_3bhk_pune_tower(habitable_floor_count=11),
            _OUT_DIR / "tier2_3bhk_pune_tower_g11.ifc",
        )
