...
import { runStripPackEngine, fillDoorMetrics } from "../src/features/floor-plan/lib/strip-pack/strip-pack-engine";
...
async function diagnose(prompt: string, runNumber: number) {
  ...
  // Step 4: Run engine
  console.log(`\n--- RUNNING STRIP-PACK ENGINE ---`);
  const engineStart = Date.now();
  const rawResult = await runStripPackEngine(parsed, prompt);
