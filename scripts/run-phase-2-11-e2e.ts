/**
 * Phase 2.11 — end-to-end measurement script.
 *
 ...
 *
 *   docs/phase-2-11-e2e-measurement.md         — human-readable report
 *   experiments/outputs/phase-2-11-e2e/        — raw artefacts:
 ...
 * Phase 2.11 target: score 70-78 / 100.
 *
 * Usage: `tsx scripts/run-phase-2-11-e2e.ts`
 */
...
const OUT_DIR = path.resolve(process.cwd(), "experiments/outputs/phase-2-11-e2e");
const REPORT_PATH = path.resolve(process.cwd(), "docs/phase-2-11-e2e-measurement.md");
...
async function main(): Promise<void> {
  ...
  console.log(`Phase 2.11 — E2E measurement`);
...
function writeReport(r: ReportInput): void {
  ...
  lines.push("# Phase 2.11 — End-to-End Quality Measurement");
  lines.push("");
  lines.push(`**Date:** ${String(rj.timestamp)}`);
  lines.push(`**Branch:** \`feat/phase-2-11-accuracy-patches\` (after all 4 steps committed)`);
  ...
  lines.push(`## 2. Phase 2.11 change-specific observations`);
  lines.push("");
  ...
  lines.push("");
  if (r.aboveTarget) {
    lines.push(`Score **${quality.score}** exceeds the Phase 2.11 target band (70–78). Phase 2.11 has delivered more than the projected ceiling on this prompt.`);
  } else if (r.inTarget) {
    lines.push(`Score **${quality.score}** lands in the Phase 2.11 target band (70–78). The four-step patch set moved the prompt out of the Phase 2.9 baseline and into the new ceiling.`);
  } else if (r.inBaseline) {
    lines.push(`Score **${quality.score}** still falls inside the Phase 2.9 baseline band (52–65). Phase 2.11's patches did not move this particular prompt. Possible reasons: (a) per-prompt variance — one sample is not a trend; (b) the weakAreas driving the score are outside Phase 2.11's scope (e.g., vastu compliance, bedroom privacy); (c) the image generation hit a known GPT-Image failure mode unrelated to label uniqueness.`);
  } else if (r.belowBaseline) {
    lines.push(`Score **${quality.score}** is below the Phase 2.9 baseline (52–65). This is a regression signal and warrants investigation before Phase 2.11 is merged.`);
  } else {
  ...
  lines.push("");
  lines.push(`**Note:** this is a single-prompt measurement. Phase 2.11's accuracy claim should be validated across 10+ prompts before declaring a production-ready ceiling shift.`);
  lines.push("");
