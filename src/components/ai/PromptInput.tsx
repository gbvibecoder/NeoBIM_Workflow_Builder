...
function matchTemplate(text: string): WorkflowTemplate {
  ...
  if (q.includes("variant") || q.includes("options"))                    return PREBUILT_WORKFLOWS.find(w => w.id === "wf-05") ?? PREBUILT_WORKFLOWS[0];
  ...
  return PREBUILT_WORKFLOWS.find(w => w.id === "wf-18") ?? PREBUILT_WORKFLOWS[0];
}
