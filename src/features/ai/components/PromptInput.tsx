...
function matchTemplate(text: string): WorkflowTemplate {
  ...
  if (q.includes("pdf") && (q.includes("mass") || q.includes("brief") || q.includes("video") || q.includes("walkthrough") || q.includes("concept"))) return PREBUILT_WORKFLOWS.find((w) => w.id === "wf-08") ?? PREBUILT_WORKFLOWS[0];
  if (q.includes("ifc") && (q.includes("quantity") || q.includes("boq"))) return PREBUILT_WORKFLOWS.find((w) => w.id === "wf-09") ?? PREBUILT_WORKFLOWS[0];
  if (q.includes("image") && q.includes("concept"))                      return PREBUILT_WORKFLOWS.find((w) => w.id === "wf-06") ?? PREBUILT_WORKFLOWS[0];
